...
def run_meeting_prep(meeting_participants: str, meeting_context: str, meeting_objective: str):
    """
    Runs the meeting prep crew with inputs coming from FastAPI—not from CLI input().
    """
    load_dotenv()
    ...
    # setting context on TASKS 
    meeting_strategy_task.context = [research_task, industry_analysis_task]
    summary_and_briefing_task.context = [meeting_strategy_task, research_task, industry_analysis_task]
...
# Keep the CLI entry, optional
def main():
    print("## Welcome to the Meeting Prep Crew")
    print('-------------------------------')
    meeting_participants = input("Participants:\n")
    meeting_context = input("Context:\n")
    meeting_objective = input("Objective:\n")
